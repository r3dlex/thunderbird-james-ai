...
Thunderbird WebExtension (Manifest V3) bringing multi-provider AI directly into the mail client. No external backend. Pure TypeScript with a Thunderbird MailExtension UI built on Vue 3 + PrimeVue + Tailwind CSS + Vite.

## Quick Reference

- **Target**: Thunderbird 148+, Manifest V3
- **Language**: TypeScript (strict mode), Python 3.12+ (tooling only)
- **UI**: Vue 3 + PrimeVue + Tailwind CSS + Vite
- **Build**: `npm run build` (webpack background + Vite UI + merge step)
- **Test**: `npm test` (Jest for background), `npm run test:ui` (Vue UI typecheck)
...
Historical migration notes and guardrails remain in [docs/vue-rewrite-phase-01.md](docs/vue-rewrite-phase-01.md) and the verification docs under `docs/`. Prefer the current source tree over older migration-era wording when they diverge.
...
| [spec/build.md](spec/build.md) | Build pipeline, webpack, Vite UI, packaging |
